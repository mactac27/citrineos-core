...
import { CalendarWithTime, type CalendarWithTimeProps } from '@lib/client/components/ui/calendar';
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <div className="flex items-center gap-1">
            ...
            {date ? formatDate(date, dateTimePickerDateFormat) : <span>{placeholder}</span>}
          </div>
      ...
      <PopoverContent className="w-auto p-6">
        <CalendarWithTime date={date ?? undefined} onSelectDateAction={onSelectDateAction} />
      </PopoverContent>
    </Popover>
  );
};
